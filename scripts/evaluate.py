...
def main(config: _config.TrainConfig, num_eval_batches: int = 10, checkpoint_step: int | None = None, checkpoint_dir: str | None = None):
    ...
    import json
    with open(str(HF_LEROBOT_HOME / Path(config.data.repo_id) / Path("meta/info.json")), 'r') as file:
        metadata = json.load(file)
        robot_type = metadata["robot_type"]

    if hasattr(config.data, 'repo_id') and config.data.repo_id:
        if 'yam' in robot_type or 'yam' in config.data.repo_id.lower():
        ...
        elif 'xmi' in robot_type or 'xmi' in config.data.repo_id.lower():
            action_names = [
